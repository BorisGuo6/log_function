...
class MLPCategoricalActor(Actor):
    ...
    def _log_prob_from_distribution(self, alg, pi, act):
        ...
        # print('act =', act)
        # print('act.unsqueeze(0) =', act.unsqueeze(0))
        # print('pi.log_prob(act) =', pi.log_prob(act))
        # print('MLPCategoricalActor(Actor)')
...
class MLPRENFORCE(nn.Module):
    ...
    def step(self, obs):
        ...
        # print('a =', a)
        # print('logp_a =', logp_a)
        # return a.detach().cpu().numpy(), logp_a
...
def train(env, policy, pi_optimizer, discount_factor, tb_writer, episode):
    ...
    while not done:
        # action_pred, log_prob_action = policy.step(torch.as_tensor(state, dtype=torch.float32).to(device))
        pi, action_pred = policy.step(torch.as_tensor(state, dtype=torch.float32).to(device))

        state, reward, done, _ = env.step(action_pred)

        # log_prob_actions.append(torch.as_tensor(log_prob_action, dtype=torch.float32).unsqueeze(0))
        pis.append(pi)
    ...
    # print('1log_prob_actions =', log_prob_actions)
    log_prob_actions = torch.stack(log_prob_actions)
    # print('2log_prob_actions =', log_prob_actions)
...
def evaluate(env, policy):
    ...
    while not done:
        with torch.no_grad():
            #  action_pred, _ = policy.step(torch.as_tensor(state, dtype=torch.float32).to(device))
